...
chi(3:ndof,:) = K(3:ndof,3:ndof)\F(3:ndof,:);% displacement
%% HOMOGENIZATION
...
CMu = diag([2 2 1]); CLambda = zeros(3); CLambda(1:2,1:2) = 1; %(5)
...
for ii=1:length(xx)
  for jj=1:length(yy)
    % Integration point
    x = xx(ii); y = yy(jj); % one gauss point
    % Differentiated shape functions
    dNx = 1/4*[-(1-y)  (1-y) (1+y) -(1+y)]; %dN/depsilon
